...
describe('generateId', () => {
  it('should generate UUID v7 when format is uuid', () => {
    const id = generateId(IdFormat.Uuid);
  ...
  it('should generate nanoid when format is nanoid with default size', () => {
    const id = generateId(IdFormat.Nanoid);
    expect(id.length).toEqual(21);
