 ...
 *
 * Reachable only when `EnvSet.values.isCloud` is `false`; Cloud runs scripts remotely. The payload
 * must be structured-cloneable — capability APIs such as `api.denyAccess` are injected by the
 * worker and must not be part of it.
 */
