...
export type StepUpEligibility = {
   ...
   * the interaction stands: every method that some path to the class needs, shortest path first.
   * Empty once the class is reached or when nothing helps.
   */
 ...
 *
 * `availableMethods` is every method that can still contribute (see {@link contributesTo}), ordered
 * by how many verifications remain after it, so a password session with an enrolled TOTP requesting
 * `mfa` is still offered the MFA subset only, while a user whose passkey reaches `mfa` alone is
 * offered the passkey first and the password + TOTP path behind it. When the class is `mfa` and no
