...
export type MfaData = {
  ...
  /**
   * Whether user skipped the optional suggestion to add passkey during the sign-up interaction.
   * This flag lives only in the current interaction and should NOT be persisted to user profile.
   */
  passkeySuggestionSkipped?: boolean;
  totp?: BindTotp;
...
export const mfaDataGuard = z.object({
  ...
  passkeySuggestionSkipped: z.boolean().optional(),
...
export class Mfa {
  ...
  constructor(
    ...
    this.#passkeySuggestionSkipped = data.passkeySuggestionSkipped;
    this.#totp = data.totp;
  ...
  /**
   * Skip the passkey suggestion for this interaction.
   * No persistence to user config.
   */
  skipPasskeySuggestion() {
    this.#passkeySuggestionSkipped = true;
  }
  ...
  async checkPasskeyAvailability() {
    ...
    if (
      ...
      !(this.#passkeySkipped ?? isPasskeySkipped(logtoConfig)) &&
      !this.#passkeySuggestionSkipped
    ) {
  ...
  get data(): MfaData {
    return {
      ...
      passkeySkipped: this.#passkeySkipped,
      passkeySuggestionSkipped: this.#passkeySuggestionSkipped,
      totp: this.#totp,
