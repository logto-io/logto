...
const passwordVerificationRecord = {
  ...
  markAsVerifiedWithUserId: jest.fn(),
};
...
const createUserForInlineHook = jest.fn();
...
type PasswordVerificationRouteContext = {
  experienceInteraction: {
    ...
    provisionLibrary: {
      createUserForInlineHook: typeof createUserForInlineHook;
...
describe('password verification route PostFirstFactorVerification fallback', () => {
  beforeEach(() => {
    ...
    createUserForInlineHook.mockResolvedValue(createdUser);
  ...
  it('preserves invalid credentials when the hook runtime returns no result', async () => {
    ...
    expect(createUserForInlineHook).not.toHaveBeenCalled();
  ...
  it('runs the inline hook create path when the local user is missing', async () => {
    ...
    expect(createUserForInlineHook).toHaveBeenCalledWith({
      primaryEmail: identifier.value,
      ...passwordHashPayload,
    });
    expect(updateUser).not.toHaveBeenCalled();
  ...
  it('runs the inline hook update path for an existing non-suspended wrong-password user', async () => {
    ...
    expect(createUserForInlineHook).not.toHaveBeenCalled();
    expect(updateUser).toHaveBeenCalledWith(activeUser.id, {
      name: 'Jane Doe',
      ...passwordHashPayload,
    });
    expect(passwordVerificationRecord.markAsVerifiedWithUserId).toHaveBeenCalledWith(
