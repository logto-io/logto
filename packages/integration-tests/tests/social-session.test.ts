...
import { mockSocialConnectorId, mockSocialConnectorConfig } from '@/__mocks__/connectors-mock';
import {
  signInWithSocial,
  getAuthWithSocial,
  registerWithSocial,
...
describe('social sign-in and register', () => {
  ...
  it('register with social', async () => {
    ...
    const response = await getAuthWithSocial(
      { connectorId: mockSocialConnectorId, data: { state, redirectUri, code, userId } },
      client.interactionCookie
...
describe('social bind account', () => {
  ...
  it('bind new social account', async () => {
    ...
    await signInWithUsernameAndPassword(username, password, client.interactionCookie);
