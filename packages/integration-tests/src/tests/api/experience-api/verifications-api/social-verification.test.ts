...
import { sendVerificationCode } from '#src/api/experience-api/verification-code.js';
...
devFeatureTest.describe('social verification', () => {
  ...
  describe('getSocialAuthorizationUri', () => {
    it('should throw if the state or redirectUri is empty', async () => {
      ...
      await expectRejects(
        client.send(getSocialAuthorizationUri, connectorId, {
          redirectUri,
          state: '',
        }),
        {
    ...
    it('should throw if the connector is not found', async () => {
      ...
      await expectRejects(
        client.send(getSocialAuthorizationUri, 'invalid_connector_id', {
          redirectUri,
          state,
        }),
        {
  ...
  describe('verifySocialAuthorization', () => {
    it('should throw if the verification record is not found', async () => {
      ...
      await expectRejects(
        client.send(verifySocialAuthorization, connectorId, {
    ...
    it('should throw if the verification type is not social', async () => {
      ...
      const { verificationId } = await client.send(sendVerificationCode, {
        identifier: {
          type: 'email',
          value: 'foo',
        },
    ...
    it('should throw if the connectorId is different', async () => {
      ...
      const { verificationId } = await client.send(getSocialAuthorizationUri, connectorId, {
        redirectUri,
        state,
      });

      await expectRejects(
        client.send(verifySocialAuthorization, 'invalid_connector_id', {
          verificationId,
          connectorData: {
            authorizationCode,
          },
