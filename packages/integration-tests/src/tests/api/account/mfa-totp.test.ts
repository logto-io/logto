...
import {
  ...
  generateTotpSecret,
  getMfaVerifications,
  createOrReplaceTotpMfaVerification,
...
describe('my-account (mfa - TOTP)', () => {
  ...
  describe('PUT /my-account/mfa-verifications/totp', () => {
    it('should be able to replace an existing totp verification', async () => {
      ...
      await createOrReplaceTotpMfaVerification(api, replaceVerificationRecordId, {
        secret: newSecret,
    ...
    it('should be able to create a new totp verification when none exists', async () => {
      await enableAllAccountCenterFields();
      ...
      await createOrReplaceTotpMfaVerification(api, verificationRecordId, {
        secret,
    ...
    it('should fail when the new totp code is invalid', async () => {
      ...
      await expectRejects(
        createOrReplaceTotpMfaVerification(api, verificationRecordId, {
          secret: newSecret,
          code: '123456',
        }),
        {
