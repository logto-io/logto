...
function SocialConnectorEditBox({ value, onChange }: Props) {
  ...
  const connectorOptions = connectorData.filter(
    ({ target, type }) => !value.includes(target) && type === ConnectorType.Blockchain
  );
  ...
  return (
    <div>
      ...
      />
      <ConnectorSetupWarning requiredConnectors={[ConnectorType.Blockchain]} />
      <div className={styles.setUpHint}>
        {t('sign_in_exp.sign_up_and_sign_in.blockchain_sign_in.set_up_hint.not_in_list')}
        <TextLink to="/connectors/social" className={styles.setup}>
          {t('sign_in_exp.sign_up_and_sign_in.blockchain_sign_in.set_up_hint.set_up_more')}
        </TextLink>
        {t('sign_in_exp.sign_up_and_sign_in.blockchain_sign_in.set_up_hint.go_to')}
      </div>
    </div>
  );
}
