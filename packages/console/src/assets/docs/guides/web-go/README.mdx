...
<InlineNotification>
  ...
  We assume your app is running on <code>http://localhost:3000</code> in this guide.  
</InlineNotification>
...
Assuming your Redirect URI is `http://localhost:3000/callback`, then we will add the `/callback` route to handle the callback after signing in.
