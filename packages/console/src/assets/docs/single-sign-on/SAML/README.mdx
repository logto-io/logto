...
The user attributes returned from IdP may vary depending on the IdP configuration. Logto provides a flexible way to map the user attributes returned from IdP to the user attributes in Logto. You can configure the user attributes mapping in the SAML SSO integration settings page.
...
Provide the email domains of your organization in the SAML SSO integration settings tab. This will enable the SSO connector as an authentication method for those users.
