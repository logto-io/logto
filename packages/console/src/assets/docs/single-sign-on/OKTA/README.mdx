...
1. Visit the OKTA admin portal and sign in as an administrator.
2. Navigate to the `Applications`/`Applications` page using the side menu.
3. Click the `Create App Integration` button to create a new OIDC application.
4. Select the `OIDC - OpenID Connect` option as the `Sign-in method`.
5. Select the `Web Application` option as the `Application type`.
