...
import { applicationTypeI18nKey, isDynamicAppRow, type DynamicAppRow } from '@/types/applications';
...
type Props = {
  readonly data:
    | Pick<Application, 'id' | 'name' | 'isThirdParty' | 'type' | 'customClientMetadata'>
    | DynamicAppRow;
};

function ApplicationPreview({ data }: Props) {
  const { t } = useTranslation(undefined, { keyPrefix: 'admin_console' });

  if (isDynamicAppRow(data)) {
    return <DynamicAppPreview />;
  }

  const { id, name, isThirdParty, type, customClientMetadata } = data;
