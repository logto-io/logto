...
beforeEach(() => {
  ...
  mockGet.mockImplementation(async (path: string) =>
    path.endsWith('/:connectorId') ? connector : []
  );
...
it('reloads authoritative state after a failed edit and permits another save', async () => {
  ...
  fireEvent.change(await screen.findByDisplayValue('Acme'), { target: { value: 'Retried' } });
  fireEvent.click(screen.getByText('Save changes'));
  await waitFor(() => {
...
it('keeps SAML SP metadata unchanged even when tenant-domain selection has a value', async () => {
  ...
  mockGet.mockImplementation(async (path: string) => (path.endsWith('/:connectorId') ? saml : []));
  openDetails('connection');
