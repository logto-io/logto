...
function CreationModal({ onClose, userId }: Props) {
  ...
  const [operation, setOperation] = useState(() => readCreation(userId));
  ...
  const create = async () => {
    ...
    try {
      const current = startCreation(userId, z.nativeEnum(SsoProviderName).parse(selected));
