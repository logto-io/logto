...
it('reuses the persisted operation after an ambiguous response and reload, then opens the created connector', async () => {
  ...
  const first = render(<CreationModal userId="alice" onClose={mockClose} />, { wrapper });
  ...
  expect(readCreation('alice')?.key).toBe('creation-key');
  first.unmount();
  render(<CreationModal userId="alice" onClose={mockClose} />, { wrapper });
  ...
  expect(readCreation('alice')).toBeUndefined();
});

it('submits directly and reports a server-side creation failure without fetching customer context', async () => {
  mockPost.mockRejectedValueOnce(new Error('No default billing account is available.'));
  render(<CreationModal userId="alice" onClose={mockClose} />, { wrapper });
  fireEvent.click(await screen.findByText('OIDC'));
  fireEvent.click(
    screen.getByRole('button', { name: 'enterprise_sso.create_modal.create_button_text' })
  ...
  await waitFor(() => {
    expect(toastResponseError).toHaveBeenCalledWith(
      expect.objectContaining({ message: 'No default billing account is available.' })
    );
  });
  expect(mockPost).toHaveBeenCalledTimes(1);
  expect(mockClose).not.toHaveBeenCalled();
  expect(readCreation('alice')?.key).toBe('creation-key');
  expect(mockGet.mock.calls.some(([path]: [string]) => path.includes('/context'))).toBe(false);
});
