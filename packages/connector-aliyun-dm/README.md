...
来到 [阿里云邮件服务页面](https://www.aliyun.com/product/directmail) 然后登录。点击页面左上的「申请开通」按钮以开通邮件服务并开始配置流程。

从 [邮件服务管理控制台页面](https://dm.console.aliyun.com/) 开始：
1. 从侧边栏进入到「发信域名」，点击「新建域名」并完成指引。
2. 依次配置好「发信地址」和「邮件标签」。
...
1. 在 [邮件服务管理控制台页面](https://dm.console.aliyun.com/)，鼠标停在右上角你的头像上，进入「AccessKey 管理」，点击「创建 AccessKey」。完成了安全验证之后，你会得到一对「AccessKey ID」和「AccessKey Secret」，请妥善保管他们。
...
3. 完成阿里云邮件服务连接器的设置：
    ...
    - 你可以添加多个邮件服务模板以应对不同的用户场景。这里展示填写单个模板的例子：
      ...
      - `usageType` 栏填写 `Register`，`SignIn` 或者 `Test` 其中之一以分别对应 _注册_，_登录_ 和 _测试_ 的不同场景。
...
你可以在「保存并完成」之前输入一个邮件地址并点击「发送」来测试配饰是否可以正常工作。
