const sign_in_experiences = {
  ...
  required_mfa_requires_non_skippable_policy:
    '自适应 MFA 和强制 MFA 需要不可跳过的 MFA 提示策略。请使用 PromptOnlyAtSignInMandatory 或 PromptAtSignInAndSignUpMandatory。',
  optional_mfa_requires_skippable_policy:
    '可选 MFA 需要可跳过的 MFA 提示策略。请勿使用 PromptOnlyAtSignInMandatory 或 PromptAtSignInAndSignUpMandatory。',
