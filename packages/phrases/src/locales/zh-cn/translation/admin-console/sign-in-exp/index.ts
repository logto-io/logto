...
const sign_in_exp = {
  ...
  account_center: {
    ...
    prebuilt_ui: {
      ...
      flows_description:
        '将你的域名与路由组合以形成账户设置 URL（例如 https://auth.foo.com/account/email）。可选择添加 `redirect=` URL 参数，以在成功更新后将用户返回到你的应用, or an `identifier=` parameter to pre-fill the identifier input field.',
      tooltips: {
        email: '更新你的主要电子邮件地址',
        phone: '更新你的主要手机号码',
        username: '更新你的用户名',
