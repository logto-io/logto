const actions = {
  ...
  environment_variables: {
    ...
    sample_code: '작업 핸들러에서 환경 변수에 접근하는 예시:',
  },
  fetch_external_data: {
    title: '외부 데이터 가져오기',
    subtitle: '작업 스크립트에서 외부 API를 호출합니다.',
    description: '`fetch` 함수로 외부 API를 호출하고 데이터를 작업 결과에 포함합니다. 예시:',
  },
  settings: {
    title: '설정',
    subtitle: '작업 활성화 여부와 런타임 오류 처리 방식을 제어합니다.',
    enabled: {
      title: '작업 활성화',
  ...
  sandbox_and_security_warning: {
    ...
    description:
      '셀프 호스팅 Logto에서는 이 스크립트가 Logto 자체와 동일한 환경에서 실행됩니다. 서버 환경 변수를 읽고 내부 네트워크의 서비스에 접근할 수 있습니다. 샌드박스되지 않습니다. 서버 접근을 허용해도 되는 사람에게만 이 페이지 접근 권한을 부여하세요. 이 작업은 로컬 비밀번호 확인이 실패한 후에만 실행됩니다. 제출된 비밀번호를 별도로 확인한 경우에만 `passwordVerified: true`를 반환하세요. 이 작업으로 프로비저닝된 사용자는 이메일 차단 목록, SSO 전용 도메인, 가입 비활성화 모드, 등록 필수 프로필 검사 등 등록 전용 제한을 우회합니다. 기존 사용자의 프로필 및 비밀번호 쓰기도 MFA 완료 전에 발생합니다.',
  },
  sandbox_warning: {
    title: '스크립트는 서버 권한으로 실행됩니다',
    description:
  ...
  delete_modal_content:
    '이 작업을 삭제하시겠습니까? 인증 흐름에서 이 스크립트가 더 이상 실행되지 않습니다.',
  deleted: '작업이 삭제되었습니다',
  created: '작업이 생성되었습니다',
  saved: '작업이 저장되었습니다',
};
