const security = {
  ...
  sentinel_policy: {
    ...
    card_description_1:
      'Блокировка доступна всем пользователям с настройками по умолчанию, но вы можете настроить её для большего контроля.',
    card_description_2:
      'Временно блокируйте идентификатор после нескольких неудачных попыток аутентификации (например, подряд неправильный пароль или код подтверждения), чтобы предотвратить грубую силу доступа.',
