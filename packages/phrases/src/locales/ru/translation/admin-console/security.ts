const security = {
  ...
  password_policy: {
    ...
    password_expiration: 'Истечение срока действия пароля',
    password_expiration_description:
      'Требовать от пользователей сброса пароля через определенное количество дней. На пользователей, входящих через SSO или ключ доступа, это не влияет.',
    enable_password_expiration: 'Включить истечение срока действия пароля',
    enable_password_expiration_description:
      'Требовать от пользователей периодически сбрасывать пароль. Существующие пользователи без записанной даты изменения пароля будут оцениваться с даты включения этой политики.',
    enable_password_expiration_tip:
      'Включить истечение срока действия пароля можно только после настройки в сценарии входа как минимум одного способа восстановления пароля с действительным коннектором.',
    ...
    password_expiration_recovery_reminder:
      'У некоторых пользователей может не быть адреса электронной почты или номера телефона для получения кода восстановления пароля, поэтому они не смогут сбросить просроченный пароль. Требуйте адрес электронной почты или номер телефона при регистрации, чтобы каждый пользователь мог восстановить свой пароль.',
  },
  verification_code_policy: {
    card_title: 'Код подтверждения',
    card_description:
      'Настройте срок действия и максимальное количество повторных попыток для кодов подтверждения, используемых при входе, регистрации и сбросе пароля.',
    enable: {
      title: 'Настроить параметры кода подтверждения',
      description:
        'Разрешить настройку срока действия кода подтверждения и максимального количества повторных попыток.',
    },
    expiration_duration: {
      title: 'Срок действия (секунды)',
      description:
        'Время в секундах, в течение которого код подтверждения остается действительным после отправки.',
      error_message: 'Срок действия должен быть от 60 до 3600 секунд.',
    },
    max_retry_attempts: {
      title: 'Максимальное количество повторных попыток',
      description:
        'Максимальное количество неудачных попыток подтверждения, разрешенное до аннулирования кода.',
  ...
  blocklist: {
    ...
    custom_email_allowlist: {
      title: 'Разрешить пользовательские адреса электронной почты',
      description:
        'Добавьте правила, разрешающие только определённые домены, адреса электронной почты или шаблоны с подстановочными символами для новых регистраций и новых привязанных адресов. Примеры: bar@example.com, @example.com, foo*@example.com, *@example.com. Домены gmail.com и googlemail.com считаются эквивалентными, а точки в локальной части игнорируются, поэтому foo.bar@gmail.com соответствует foobar@googlemail.com.',
      placeholder: 'Введите адрес электронной почты, домен или шаблон с подстановочным символом',
      duplicate_error:
        'Адрес электронной почты, домен или шаблон с подстановочным символом уже добавлен',
      invalid_format_error:
        'Должен быть действительный адрес электронной почты (bar@example.com), домен (@example.com) или шаблон с подстановочным символом (foo*@example.com, *@example.com)',
      warnings: {
        identical_entries:
          'Некоторые записи списка разрешений также присутствуют в правилах блокировки. Совпадающие адреса могут по-прежнему блокироваться.',
        blocked_exact_email:
          'Некоторые точные адреса из списка разрешений совпадают с правилом блокировки. Совпадающие адреса могут по-прежнему блокироваться.',
        blocked_subaddressing:
          'Некоторые записи списка разрешений содержат знак плюс (+), но субадресация электронной почты заблокирована.',
        effectively_unusable:
          'Согласно этим проверкам, текущий список разрешений может не пропустить ни один новый адрес электронной почты.',
      },
    },
    disposable_email: {
    ...
    custom_email_address: {
      ...
      description:
        'Добавьте правила, запрещающие определённым доменам, адресам электронной почты или шаблонам с подстановочными символами регистрироваться или связываться через пользовательский интерфейс. Примеры: bar@example.com, @example.com, foo*@example.com, *@example.com. Домены gmail.com и googlemail.com считаются эквивалентными, а точки в локальной части игнорируются, поэтому foo.bar@gmail.com соответствует foobar@googlemail.com.',
      placeholder: 'Введите адрес электронной почты, домен или шаблон с подстановочным символом',
      duplicate_error:
        'Адрес электронной почты, домен или шаблон адреса с подстановочным знаком уже добавлен',
      invalid_format_error:
        'Должен быть действительным адресом электронной почты (bar@example.com), доменом (@example.com) или шаблоном адреса электронной почты с подстановочным знаком (foo*@example.com, *@example.com)',
    },
  },
};
