const security = {
  ...
  blocklist: {
    ...
    email_subaddressing: {
      ...
      description:
        'Включите, чтобы предотвратить использование пользователями субадресов электронной почты с плюсом (+) и дополнительными символами (например, user+alias@foo.com).',
    },
