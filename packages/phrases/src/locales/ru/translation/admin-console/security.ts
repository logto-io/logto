const security = {
  ...
  password_policy: {
    ...
    enable_password_expiration_description:
      'Требовать от пользователей периодически сбрасывать пароль. Существующие пользователи без записанной даты изменения пароля будут оцениваться по дате создания их учетной записи.',
    ...
    expiration_reminder: 'Напоминание об истечении срока действия (за сколько дней до истечения)',
    expiration_reminder_description:
      'Количество дней до истечения срока действия для предупреждения пользователей и предложения сбросить пароль. Пользователи могут пропустить напоминание и продолжить вход.',
    expiration_reminder_error:
      'Период напоминания об истечении срока действия должен составлять от 0 до срока действия ({{max}} дней).',
  },
  sentinel_policy: {
