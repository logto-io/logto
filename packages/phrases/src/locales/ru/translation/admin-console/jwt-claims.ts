const jwt_claims = {
  ...
  user_jwt: {
    ...
    custom_data_in_id_token: {
      title: 'Добавить пользовательские данные в токен ID',
      tip: 'Токен ID - это утверждение идентичности, которое клиент получает после входа, чтобы доказать, кто является пользователем. Он предназначен для клиента, содержит утверждения идентичности пользователя для отображения или создания сеанса.',
      description:
        'Включите, чтобы добавить ваши <a>пользовательские данные</a> в токен ID, позволяя клиентам немедленно получать доступ к этим деталям идентичности без дополнительных вызовов профиля.',
    },
  },
