const applications = {
  ...
  dynamic_app: {
    ...
    beta_notice:
      'Динамическое приложение сейчас находится в бета-тестировании. Добро пожаловать, чтобы изучить его и <ContactLink>поделиться своими отзывами</ContactLink>.',
    ...
    enable_confirm_modal: {
      ...
      beta_pricing_notice:
        'Динамическое приложение бесплатно в период бета-тестирования. После его завершения может взиматься плата за дополнение. Мы предупредим вас заранее, и вы сможете отключить функцию в любой момент.',
    },
    ...
    permissions: {
      ...
      user_description:
        'Выберите разрешения, запрашиваемые клиентами OAuth для доступа к определенным данным пользователя.',
      grant_user_level_permissions: 'Предоставить разрешения пользователя',
      organization_title: 'Организация',
      organization_description:
        'Выберите разрешения, запрашиваемые клиентами OAuth для доступа к определенным данным организации.',
      grant_organization_level_permissions: 'Предоставить разрешения организации',
      permission_delete_confirm:
        'Это действие удалит разрешение из динамического приложения, из-за чего клиенты OAuth не смогут запрашивать авторизацию пользователя для него. Вы уверены, что хотите продолжить?',
    },
    client_compatibility: {
      title: 'Совместимость с клиентами',
      description:
        'Настройте, как Logto обрабатывает запросы авторизации от OAuth-клиентов, которые пока не полностью поддерживают OpenID Connect. Эти настройки экспериментальные и могут быть изменены или удалены.',
      add_consent_prompt_for_offline_access: 'Добавлять запрос согласия для офлайн-доступа',
      add_consent_prompt_for_offline_access_description:
        'Когда OAuth-клиент запрашивает `offline_access` без `prompt=consent`, Logto добавляет запрос согласия, чтобы клиент мог получить Refresh Token. Рекомендуется для таких клиентов, как ChatGPT и Codex.',
    },
  },
  guide: {
    third_party: {
