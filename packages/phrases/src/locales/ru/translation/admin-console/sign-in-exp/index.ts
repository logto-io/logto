...
const sign_in_exp = {
  ...
  account_center: {
    ...
    prebuilt_ui: {
      ...
      flows_description:
        'Объедините ваш домен с маршрутом, чтобы сформировать URL настроек аккаунта (например, https://auth.foo.com/account/email). При желании добавьте параметр URL `redirect=` для возврата пользователей в ваше приложение после успешного обновления, или параметр `user_id=` для проверки входа правильного пользователя (автоматический повторный вход при несоответствии).',
      tooltips: {
