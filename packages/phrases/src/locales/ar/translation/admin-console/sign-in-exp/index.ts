...
const sign_in_exp = {
  ...
  account_center: {
    ...
    prebuilt_ui: {
      ...
      flows_description:
        'اتخذ نطاقك وأضفه إلى المسار لتشكل عنوان URL لإعدادات حسابك (مثل، https://auth.foo.com/account/email). بشكل اختياري، أضف معامل `redirect=` URL لإرجاع المستخدمين إلى تطبيقك بعد التحديث بنجاح، أو معامل `user_id=` للتأكد من تسجيل دخول المستخدم الصحيح (إعادة تسجيل الدخول تلقائيًا في حالة عدم التطابق).',
      tooltips: {
