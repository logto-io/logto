const actions = {
  ...
  sandbox_and_security_warning: {
    ...
    description:
      'セルフホスト版 Logto では、このスクリプトは Logto 本体と同じ環境で実行されます。サーバーの環境変数を読み取り、内部ネットワーク上のサービスに到達できます。サンドボックス化されていません。サーバーへのアクセスを許可してもよい相手にのみ、このページへのアクセスを付与してください。このアクションは、ローカルパスワードの検証が失敗した後にのみ実行されます。送信されたパスワードを別途検証した場合にのみ `passwordVerified: true` を返してください。このアクションでプロビジョニングされたユーザーは、メールブロックリスト、SSO 専用ドメイン、サインアップ無効モード、登録時の必須プロフィールチェックなど、登録専用の制限をバイパスします。既存ユーザーのプロフィールとパスワードの書き込みも、MFA 完了前に行われます。',
  },
  sandbox_warning: {
    title: 'スクリプトはサーバー権限で実行されます',
    description:
