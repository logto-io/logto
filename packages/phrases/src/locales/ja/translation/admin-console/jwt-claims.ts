const jwt_claims = {
  ...
  user_jwt: {
    ...
    custom_data_in_id_token: {
      title: 'IDトークンにユーザーカスタムデータを追加',
      tip: 'IDトークンは、サインイン後にクライアントが受け取る本人確認のためのアサーションで、ユーザーが誰であるかを証明します。クライアント向けで、表示またはセッション作成のためのユーザーIDクレームが含まれます。',
      description:
        '有効にすると、<a>ユーザーカスタムデータ</a>をIDトークンに含めることができ、クライアントは追加のプロファイル呼び出しなしでこれらのID詳細に即座にアクセスできます。',
    },
  },
