...
const sign_in_exp = {
  ...
  account_center: {
    ...
    prebuilt_ui: {
      ...
      flows_description:
        '結合你的域名與路徑形成你的帳戶設置 URL（例如，https://auth.foo.com/account/email）。可以選擇性地添加一個 `redirect=` URL 參數來在成功更新後返回用戶至你的應用程式, or an `identifier=` parameter to pre-fill the identifier input field.',
      tooltips: {
        email: '更新你的主電郵地址',
        phone: '更新你的主電話號碼',
        username: '更新你的使用者名稱',
