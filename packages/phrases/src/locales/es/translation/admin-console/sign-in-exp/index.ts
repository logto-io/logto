...
const sign_in_exp = {
  ...
  account_center: {
    ...
    prebuilt_ui: {
      ...
      flows_description:
        'Combina tu dominio con la ruta para formar tu URL de configuración de cuenta (por ejemplo, https://auth.foo.com/account/email). Opcionalmente, agrega un parámetro de URL `redirect=` para devolver a los usuarios a tu aplicación después de actualizar con éxito, o un parámetro `user_id=` para asegurar que el usuario correcto haya iniciado sesión (reinicio de sesión automático si no coincide).',
      tooltips: {
